...
def init_db():
    with sqlite3.connect(DB_PATH) as con:
        ...
        """)
        # таблица для рассылки (нужна коду рассылки)
        con.execute("""
...
def complete_order(order_id: int, result_link: str):
    with sqlite3.connect(DB_PATH) as con:
        con.execute(
            "UPDATE orders SET status='done', result_link=? WHERE order_id=?",
...
async def cmd_start(msg: Message):
    register_user(msg.from_user.id, msg.from_user.username or "")
